...
function students_guard($new_status, $old_status, $post) {
	if ($new_status === 'publish' 
		...
		wp_die(__('Tidak dapat memposting, silahkan tambahkan featured image terlebih dahulu.', wp_template));
	}
}
...
function students_enqueue_edit_screen_js($hook) {
	...
	if (in_array($post->post_type, students_return_post_types())) {
		...
		wp_localize_script(
			...
			'objectL10n',
			array(
				'jsWarningHtml' => __('<strong>Featured image belum dipilih</strong>, silahkan tambahkan featured image.', wp_template),
			)
		);
	}
}
